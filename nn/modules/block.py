...
from .conv import Conv, DWConv, GhostConv, LightConv, RepConv, autopad, CBAM, FDConv
...
class C2f_CBAM(C2f):
    """Faster Implementation of CSP Bottleneck with 2 convolutions, integrated with CBAM for attention."""

    def __init__(self, c1, c2, n=1, shortcut=False, g=1, e=0.5):
        """Initializes a C2f module, adds CBAM, ensuring correct chunk size for concatenation."""
        super().__init__(c1, c2, n, shortcut, g, e)
        # self.c 和 self.m 已经在 super().__init__() 中初始化
        
        # 定义 CBAM 模块，应用于最终输出之前
        self.cbam = CBAM(c2) 
    ...
    def forward_split(self, x):
        """Forward pass using split() instead of chunk(), integrated with CBAM."""
        y = list(self.cv1(x).split((self.c, self.c), 1))
        y.extend(m(y[-1]) for m in self.m)
        x_out = self.cv2(torch.cat(y, 1))
