...
class StarReLU(nn.Module):
    ...
    def forward(self, x):
        return self.scale.to('cuda:0') * self.relu(x).to('cuda:0') ** 2 + self.bias.to('cuda:0')
...
class FrequencyBandModulation(nn.Module):
    ...
    def forward(self, x, att_feat=None):
        ...
        for idx, freq in enumerate(self.k_list):
            # 直接从缓存中获取mask
            mask = current_masks[idx].to('cuda:0')
            ...
            # 将注意力权重和高频部分相乘
            # 重塑形状以进行广播
            tmp = freq_weight.reshape(b, self.spatial_group, -1, h, w).to('cuda:0') * \
                  high_part.reshape(b, self.spatial_group, -1, h, w).to('cuda:0')
...
class FDConv(nn.Conv2d):
    ...
    def forward(self, x):
        ...
        if self.use_ksm_local:
            ...
            hr_att_logit = self.KSM_Local(global_x.to('cuda:0')) # b, kn, cin, cout * ratio, k1*k2, 
        ...
        for i in range(self.param_ratio):
            ...
            if self.param_reduction < 1:
                ...
                # print(w.shape)
                DFT_map[:, indices[0, :, :], indices[1, :, :]] += torch.stack([w[..., 0].to('cuda:0') * kernel_attention[:, i], w[..., 1].to('cuda:0') * kernel_attention[:, i]], dim=-1)
                pass
        ...
        if self.out_channels * self.in_channels * self.kernel_size[0] * self.kernel_size[1] < (in_planes + self.out_channels) * height * width:
            ...
            aggregate_weight = spatial_attention.to('cuda:0') * channel_attention.to('cuda:0') * filter_attention.to('cuda:0') * adaptive_weights.to('cuda:0') * hr_att.to('cuda:0')
        ...
        if self.bias is not None:
            output = output.to('cuda:0') + self.bias.view(1, -1, 1, 1).to('cuda:0')
        return output
